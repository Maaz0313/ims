...
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Edit Purchase Order #{{ $purchaseOrder->po_number }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('purchase-orders.update', $purchaseOrder) }}" method="POST" id="purchase-order-form">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('order_date') is-invalid @enderror" id="order_date" name="order_date" value="{{ old('order_date', $purchaseOrder->order_date->format('Y-m-d')) }}" required>
                        @error('order_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="table-responsive mb-3">
                <table class="table table-bordered" id="products-table">
                    ...
                    <tbody>
                        <tr id="empty-row" class="{{ count($purchaseOrder->items) > 0 ? 'd-none' : '' }}">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="text" id="grand-total" class="form-control" value="{{ number_format($purchaseOrder->total_amount, 2) }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
        ...
        addItemBtn.addEventListener('click', function() {
            ...
            for (let i = 0; i < existingRows.length; i++) {
                ...
                if (productSelect && productSelect.value === productId) {
                    ...
                    itemTotalInput.value = (parseFloat(unitPriceInput.value) * parseInt(quantityInput.value)).toFixed(2);
