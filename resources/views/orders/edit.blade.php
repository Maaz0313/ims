...
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Edit Order #{{ $order->order_number }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('orders.update', $order) }}" method="POST" id="order-form">
            ...
            <div class="table-responsive mb-3">
                <table class="table table-bordered" id="products-table">
                    ...
                    <tbody>
                        <tr id="empty-row" class="{{ count($order->items) > 0 ? 'd-none' : '' }}">
                            <td colspan="5" class="text-center">No products added. Use the form below to add products.</td>
                        </tr>
                            ...
                            <tr class="product-row">
                                    ...
                                    <select name="products[{{ $index }}][product_id]" class="form-select product-select" required>
                                        ...
                                        @foreach($products as $product)
                                            ...
                                            <option value="{{ $product->id }}" {{ $item->product_id == $product->id ? 'selected' : '' }}
                                                data-price="{{ $product->price }}" data-max="{{ $maxQuantity }}">
                                                {{ $product->name }} ({{ $product->sku }}) - {{ $maxQuantity }} available
                                            </option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="products[{{ $index }}][unit_price]" class="form-control unit-price" step="0.01" min="0" value="{{ $item->unit_price }}" required>
                                    </div>
                                </td>
                                <td>
                                    <input type="number" name="products[{{ $index }}][quantity]" class="form-control quantity" min="1" value="{{ $item->quantity }}" required>
                                </td>
                                <td>
                                    <div class="input-group">
                                        <span class="input-group-text">$</span>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="text" id="subtotal" class="form-control" value="{{ number_format($order->total_amount, 2) }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end">
                                <div class="input-group" style="max-width: 150px; float: right;">
                                    <input type="number" id="tax-rate" name="tax_rate" class="form-control" min="0" max="100" step="0.01" value="{{ old('tax_rate', $order->tax_amount > 0 ? ($order->tax_amount / $order->total_amount) * 100 : 0) }}">
                                    <span class="input-group-text">%</span>
                                </div>
                            ...
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="text" id="tax-amount" class="form-control" value="{{ number_format($order->tax_amount, 2) }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end">
                                <div class="input-group" style="max-width: 150px; float: right;">
                                    <span class="input-group-text">$</span>
                                    <input type="number" id="discount-amount" name="discount_amount" class="form-control" min="0" step="0.01" value="{{ old('discount_amount', $order->discount_amount) }}">
                                </div>
                            ...
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="text" id="discount-display" class="form-control" value="{{ number_format($order->discount_amount, 2) }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="text" id="grand-total" class="form-control" value="{{ number_format($order->grand_total, 2) }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                            ...
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="add-product">
                                            <option value="">Select Product</option>
                                            @foreach($products as $product)
                                                <option value="{{ $product->id }}" data-price="{{ $product->price }}" data-max="{{ $product->inventory->quantity }}">
                                                    {{ $product->name }} ({{ $product->sku }}) - {{ $product->inventory->quantity }} in stock
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-text">$</span>
                                            <input type="number" class="form-control" id="add-price" step="0.01" min="0" value="0.00">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addItemBtn.addEventListener('click', function() {
            ...
            const maxQuantity = parseInt(addProductSelect.options[addProductSelect.selectedIndex].getAttribute('data-max'));
            ...
            for (let i = 0; i < existingRows.length; i++) {
                ...
                if (productSelect && productSelect.value === productId) {
                    ...
                    if (newQuantity > maxQuantity) {
                        alert(`Cannot add ${quantity} more units. Maximum available quantity is ${maxQuantity}. You already have ${currentQuantity} in your order.`);
                        return;
                    }
